...
@Stateless
@Local(Service.class)
public class CellKPIDrillDownService extends AbstractKPIService {
    ...
    public String getData(final MultivaluedMap<String, String> serviceProviderParameters) {
        ...
        try {
            cellNames = rncCellKPIServiceParamUtil.extractCellIDFromURLParam(serviceProviderParameters.getFirst(SEARCH_PARAM));
            ...
            if (version.trim().equals(BIS_SOAP_SERVICES_VERSION_SOAP4)) {
                nodeQueryKPIDataTypes = nodeQuery.getCellKPIDrillDownSoap4(cellNames, xmlGregorianCalendar);
            } else {
                nodeQueryKPIDataTypes = nodeQuery.getCellKPIDrillDownSoap3(cellNames, xmlGregorianCalendar);
            }
            ...
            return handleKPIServiceException(e, CellKPIDrillDownService.class.getName());
        }
    }
}
