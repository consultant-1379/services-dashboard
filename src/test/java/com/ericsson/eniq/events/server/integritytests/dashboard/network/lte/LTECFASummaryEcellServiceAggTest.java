...
import com.ericsson.eniq.events.server.serviceprovider.impl.dashboard.network.lte.LTECFASummaryEcellService;
import com.ericsson.eniq.events.server.test.queryresults.dashboard.network.lte.LTECFASummaryEcellResult;
 ...
 *
 */
public class LTECFASummaryEcellServiceAggTest extends BaseDataIntegrityTest<LTECFASummaryEcellResult> {

    private LTECFASummaryEcellService lteCfaSummaryEcellService;

    private final String temporaryTable = "#EVENT_E_LTE_CFA_HIER321_ERR_DAY";

    @Before
    public void onSetup() {
        lteCfaSummaryEcellService = new LTECFASummaryEcellService();
        attachDependencies(lteCfaSummaryEcellService);
        try {
            createTable();
    ...
    public void testRunQuery() throws Exception {
        ...
        final String result = runQuery(lteCfaSummaryEcellService, requestParameters);
        verifyResult(result);
    }
    ...
    private void verifyResult(final String jsonResult) throws Exception {
        final List<LTECFASummaryEcellResult> results = getTranslator().translateResult(jsonResult,
                LTECFASummaryEcellResult.class);
        assertEquals(5, results.size());

        LTECFASummaryEcellResult resultFromQuery = results.get(0);
